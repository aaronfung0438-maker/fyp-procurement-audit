...
### 3.4 實驗後信任量表（全組皆填，G1 用 Section E 平行版）

**所有三組**在完成 32 道正式題後皆填寫一份 5 題的信任量表，改編自 Jian, Bisantz & Drury (2000)「Checklist for Trust between People and Automation」實證驗證的 12-item 信任量表。Q1 / Q2 / Q4 改編自原量表的 reliability / dependability / security 三個面向；Q3 與 Q5 為本研究新增，分別針對 RQ2 的「證據型 vs. 結論型 AI 輸出之清晰度感知差異」與 RQ3 的「自動化偏誤主觀知覺」設計。

**兩個平行 instrument**：
- **AI 版（G2 / G3）**：五題的主詞為 "the AI system" / "the AI"。
- **Section E 版（G1）**：五題的主詞替換為 "Section E (the deviation sentences)"，construct 完全對應，僅變更資訊來源指稱。

設計理由：原本「G1 直接跳 Thanks」的方案會導致主觀 DV 完全沒有 G1 資料，使 §14 的三組比較僅能在客觀 DV 層面進行，浪費 G1 樣本可提供的「非 AI 條件下對統計式資訊源的信任」基線資訊。改為平行 instrument 後：
- G1 vs. G2/G3 可比較「結構化統計資訊（Section E）」與「LLM 輸出」這兩種資訊源在同一組信任 / 清晰度 / 有用性 / 自主感面向上的主觀差異
- 仍維持 G2 vs. G3 的 AI 內部比較（同一份 instrument，僅 AI 輸出形式差異）
- G1 受試者不需要「想像有 AI」，避免 demand effect

**評分**：每題使用 7-point Likert（1 = Strongly disagree, 4 = Neutral, 7 = Strongly agree）。App 顯示英文版作為正式 instrument；中譯僅作為論文敘述參考，不向受試者呈現。

#### 3.4.1 量表題目

##### 3.4.1a　AI 版（用於 G2、G3）
...
##### 3.4.1b　Section E 版（用於 G1）

| # | Question (English — used in app) | 中譯（僅供參考）| Construct |
|---|---|---|---|
| Q1 | I can trust the information shown in Section E (the deviation sentences). | 我相信 Section E（差異句）所呈現的資訊是可靠的。| Trust — reliability |
| Q2 | The Section E deviation sentences were dependable enough for me to rely on when making my decision. | Section E 的差異句穩健到足以讓我依賴它做決定。| Trust — dependability |
| Q3 | The Section E deviation sentences made it clear to me why an order might be noteworthy. | Section E 的差異句讓我清楚理解為何某份訂單值得注意。| Clarity / explanation quality |
| Q4 | Section E provided substantive help to my final judgment. | Section E 對我的最終判斷有實質幫助。| Perceived usefulness |
| Q5 | I made the final decision myself based on my own analysis, rather than simply following whatever Section E suggested. | 我根據自己的分析做出最終決定，而不是跟著 Section E 給的方向走。| Perceived autonomy / automation-bias awareness |

兩版的 Q1–Q5 share 相同的 5 個 construct（reliability / dependability / clarity / usefulness / autonomy），僅將「資訊源」由 "the AI" 替換為 "Section E"，使 G1 vs. G2/G3 的主觀比較具有 construct-level 對應關係。

#### 3.4.2 量表呈現方式（App UI）

- **單頁呈現**：5 題顯示在同一頁面同一張表單中；受試者完成所有 5 題後一次性按 **Submit** 提交，不採用「逐題提交、無法回頭」的設計。理由：5 題之間具有結構相關性（同一個 AI 系統的不同面向），允許受試者在填寫後幾題時往前修改前幾題的答案，較能避免序位效應（order effect）。
- **題序固定**：Q1 → Q5，**不隨機化**。理由：5 題量太小，固定題序便於跨人比較；且 Q1/Q2 為總體信任，Q3/Q4/Q5 為更具體的操縱知覺，由抽象到具體的順序符合認知負荷遞減原則。
- **Likert anchor 顯示**：每個 1–7 的選項都顯示完整描述：`1 — Strongly disagree`、`2`、`3`、`4 — Neutral`、`5`、`6`、`7 — Strongly agree`，採水平 radio button 排列。
- **預設選項**：每題預設停在中間值 4（Neutral），受試者必須主動選擇才會偏移。預設值的選擇有偏誤風險（可能誘發 central tendency bias），但相對於「無預設」（強迫互動）能降低未填遺漏；此處取捨後選擇有預設值。
- **時間估計**：5 題實測完成時間約 **90–120 秒**。
- **無 attention check**：本研究 N=8（G2+G3）樣本下，attention check 的訊號雜訊比過低（單題誤答即可能損失 12.5% 樣本），故不納入。

#### 3.4.3 計分與資料儲存

- **計分方向**：5 題**全部為正向題**（高分 = 高信任/高清晰/高有用/高自主感）。原 Jian et al. (2000) 量表的 5 題負向題（"deceptive"、"underhanded"、"suspicious"、"wary"、"harmful"）已於簡化階段移除（見 §3.4.5），因此**不存在反向計分**步驟。
- **量表分數**：本研究**不計算單一總分**或內部一致性 α。每題作為獨立 DV 報告，理由：
  1. N=4/組的樣本下，5 題加總的可靠性估計（Cronbach's α）信賴區間極寬，無法解釋。
  2. Q3/Q5 為 RQ-specific 的新題，與 Q1/Q2/Q4 的構念並非單一信任維度，加總會掩蓋本研究最關注的差異。
- **儲存欄位**（Google Sheets `sessions` 工作表，見 §13.x）：
  - 每位受試者的 5 題答案以 **5 個獨立 row** 寫入，欄位對應如下：
    | 欄位 | 內容 |
    |---|---|
    | `participant_id` | 受試者 ID（normalised name）|
    | `group` | `G1` / `G2` / `G3` |
    | `phase` | `trust_survey`（固定值）|
    | `question_idx` | `Q1` / `Q2` / `Q3` / `Q4` / `Q5`（題號字串）|
    | `po_id` | 空字串（信任量表非針對特定 PO）|
    | `submit_ts` | 提交時的 ISO 時間戳（5 題共用同一個 `submit_ts`）|
    | `confidence` | 該題的 Likert 分數（整數 1–7）|
    | `judgment` / `rationale` / `render_ts` / `duration_ms` | 空字串 |
    | `extra_json` | `{"item_id": "Q1", "construct": "trust_reliability", "instrument": "ai" \| "section_e", "text": "..."}` |
- `extra_json.instrument` 區分受試者填的是 AI 版（`"ai"`，G2 / G3）或 Section E 版（`"section_e"`，G1），分析時依此分流。
- 採用「5 個 row」而非「1 個 row 含 5 個欄位」的理由：保持 `sessions` 工作表的單一 schema（每個觀察值 = 1 row），避免為信任量表額外開一張工作表。

#### 3.4.4 在分析中的角色

主觀 DV 的組間比較分為兩層，對應不同的研究問題：

**層 1 — G2 vs. G3（同為 AI 版 instrument）**：
- 直接測試 RQ2、RQ3 的核心操縱效應（AI 結論型 vs. AI 證據型）。
- 例如「G3 的 Q3 平均分數是否顯著高於 G2」可作為「證據型輸出較易理解」的主觀證據；「G3 的 Q5 平均分數是否高於 G2」可作為「證據型輸出較不誘發自動化偏誤」的主觀證據。
- 此層的兩組填的是**同一份** instrument（"the AI ..."），跨組比較具有完全的 construct 對應性。

**層 2 — G1 vs. (G2 ∪ G3)（跨 instrument 平行比較）**：
- 比較「結構化統計資訊源（Section E）」與「LLM 輸出資訊源（AI）」在同一組信任 / 清晰度 / 有用性 / 自主感面向上的主觀差異。
- 此層為**探索性**分析：兩組填的是不同 instrument（主詞替換），雖然 construct 對應，但 stimulus 本質不同（自然語言 vs. 計算句），故結論需謹慎詮釋為「非實驗性的描述差異」而非「因果差異」。
- 主要用於回答補充問題：「人類在無 AI 條件下對 Section E 的信任水準是否本就較高 / 較低？」此基線資訊有助於解讀 G2 / G3 的 AI 信任分數絕對值。

**統計檢定**：
- Mann–Whitney U test（非常態小樣本）+ Cliff's δ 報效應大小，與 §14.4 主分析方法一致。
- 每題獨立檢定。多重比較校正採 Holm-Bonferroni：層 1 內 5 個檢定校正一次，層 2 內 5 個檢定校正一次（兩層分開校正以保留各自的解釋力）。
- 客觀 DV（決策正確率、與 AI 一致率）與主觀 DV（Likert 分數）若方向一致 → 證據強化；若方向不一致 → 於 Discussion 章節進行三角驗證討論（例如「客觀上 G3 比 G2 更不依賴 AI，但主觀 Q5 分數無差異」可解讀為自動化偏誤的非自覺特性）。
...
#### 3.4.6 G1 採用 Section E 平行版的設計考量

**問題**：原 AI 版 instrument 的 5 題全部以 "the AI" 為主詞，G1 條件下不存在 AI 輸出可供評價。

**初版方案（已捨棄）**：讓 G1 直接跳過信任量表，主觀 DV 完全沒有 G1 資料。但此方案有兩個明顯缺點：
- 浪費 G1 樣本本可提供的「非 AI 條件下對統計式資訊源的信任基線」資訊
- §14 的三組比較被迫只能在客觀 DV 層面進行，主觀面向僅剩 G2 vs. G3，論文中的「三組設計」其實退化為「兩組設計加一個對照組」

**最終採用方案（Section E 平行版）**：將 5 題的主詞由 "the AI" 替換為 "Section E (the deviation sentences)"，construct 完全對應（reliability / dependability / clarity / usefulness / autonomy），但資訊源換成 G1 真正接觸過的 Section E。理由：
- G1 受試者**真實使用過** Section E（每題都看），不需要「想像」任何未經歷的內容，不引入 demand effect
- 維持「全組皆填」的 schema 一致性，使 §14 的主觀 DV 分析可在三組層面進行
- 平行 instrument 的設計在 HCI 文獻中常見（例如比較不同決策輔助工具的信任時，會依工具替換 stimulus 主詞）

**取捨揭露**：兩版 instrument 的 stimulus 本質不同（LLM 自然語言 vs. 計算式統計句），跨 instrument 的分數差異**不能**直接詮釋為「人類更信任 X 而非 Y」的因果結論，僅能視為「在當前實驗條件下，這兩個資訊源誘發的主觀感受存在 / 不存在差異」。此限制於 §15.x 一併揭露。
...
### 15.11 信任量表跨 instrument 比較的詮釋限制

§3.4 採用兩個平行的信任量表 instrument：AI 版（G2 / G3）與 Section E 版（G1）。兩版的 5 題 construct 完全對應（reliability / dependability / clarity / usefulness / autonomy），僅將「資訊源」由 "the AI" 替換為 "Section E (the deviation sentences)"。這個設計使全組皆可填寫主觀 DV，但在跨 instrument 的層 2 比較（G1 vs. G2 ∪ G3）有以下限制：

- **stimulus 本質不同**：AI 輸出為自然語言推理（G2 verdict / G3 evidence list），Section E 為固定模板的計算式統計句。兩者誘發的認知處理可能根本不同（敘事 vs. 數值），故信任分數的差異無法純粹歸因於「資訊源類別」。
- **無 instrument-equivalence 驗證**：本研究未進行 measurement invariance 測試（例如多群組 CFA），無法統計上保證兩版 instrument 在受試者心中觸發的潛在構念完全相同。N=12 樣本下也無法做此檢定。
- **詮釋邊界**：層 2 的結論僅能描述為「在當前實驗條件下，G1 對 Section E 的主觀評價 vs. G2/G3 對 AI 的主觀評價，於某面向上存在 / 不存在差異」。**不能**直接推論「人類更信任 X 資訊源而非 Y」，因為差異可能來自 instrument wording、stimulus 形式、或兩者交互作用。
- **層 1 不受影響**：G2 vs. G3 的比較雙方填同一份 AI 版 instrument，construct 與 stimulus wording 完全對應，是本研究主觀 DV 的主要分析。層 2 為次級 / 探索性分析。

後續工作建議：若有更大樣本（N ≥ 30），可加入第四組「同時看 Section E 與 AI 輸出，分別填兩份 instrument」作為 within-subject 條件，以分離「資訊源類別」與「instrument wording」兩個混淆因子。
